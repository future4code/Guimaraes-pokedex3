...
const GlobalState =  (props) => {
    ...
    useEffect(() => {
        const list = []
        pokemonData && pokemonData.results.map((poke) => {
            list.push(poke.url)
    ...
    useEffect(() => {
        const dataPokemon = []
        pokemonsUrls && pokemonsUrls.map((poke) => {
            axios
            .get(poke)
            .then((res) => {
    ...
    const addToPokedex = (poke) => {
        const pokeIndex = pokemonsDetails.findIndex((pokemon) => pokemon.name === poke.name)
    ...
    const rmvPokemonCart = (poke) => {
        const pokeIndex = pokedex.findIndex((pokemon) => pokemon.name === poke.name)
